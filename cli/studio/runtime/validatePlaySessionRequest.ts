...
// `seed`, unlike validateRuntimeSessionRequest's own `initialBalance` -- a Play session's starting
// credits always come from the game's own session initialization (see StudioPlayService.newSession()'s
// own doc comment), so there's no pre-generated-library "fund it up front" case to accept a field for
// here.
