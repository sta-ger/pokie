...
// The postMessage protocol a worker thread speaks to SimulationWorkerCoordinator — a tagged union so
// the coordinator never has to guess what an incoming message means or rely on uncaught-exception
// serialization for error reporting (see simulationWorkerEntry.ts, which never lets an exception
// escape uncaught: every failure is turned into an explicit {type: "error"} message with a safe,
// stack-trace-free reason).
