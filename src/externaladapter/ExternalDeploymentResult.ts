...
// (descriptor validation -> compatibility -> generation -> artifact validation -> optional diagnostic ->
// optional delivery) got, and why. Every stage after the first one that reported an error-severity issue is
// simply absent (`undefined`) rather than populated with empty placeholder data, so "did generation even run"
// is always answerable by checking `generation !== undefined`, never by inspecting whether some nested array
// happens to be empty (an empty "artifacts"/"issues" array is itself a perfectly valid, meaningful result of a
// stage that *did* run).
