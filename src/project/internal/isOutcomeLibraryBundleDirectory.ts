...
// A lightweight, non-throwing recognition check for ProjectTargetResolver's outcomeLibrary adapter —
// deliberately not a structural validation pass (OutcomeLibraryBundleValidator already owns that, against an
// already-recognized bundle directory): just enough of manifest.json's own shape to tell "this directory is an
// outcome-library bundle" apart from any other directory, the same "recognize, don't validate" split
...
// or doesn't look like OutcomeLibraryBundleManifest — a malformed manifest.json is simply not recognized, not
// an error.
