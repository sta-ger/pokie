...
//
// Deliberately just (session) here, with no bet-mode parameter: this is a public contract, and any
// external code with its own ForcedFeatureEntryHandling implementation, or that calls
// canForceFeatureEntry(session)/forceFeatureEntry(session) directly on a value typed as this interface,
// must keep compiling. A handler that DOES need to know which mode triggered forced entry (e.g. to
// route several differently-priced/differently-granting buyFeature modes -- see
// PerModeForcedFeatureEntryHandler) additionally implements ModeAwareForcedFeatureEntryHandling, a
// separate, additive extension of this one; VideoSlotWithBetModesSession.play() feature-detects it and
// prefers it when present, falling back to the plain methods here otherwise.
