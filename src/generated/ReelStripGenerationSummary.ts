...
export type ReelStripGenerationSummary = {
    reelIndex: number;
    // The exact authored config that drove this reel's generation (including its own seed) --
    // preserved verbatim for provenance, independent of whether generation succeeded.
    config: ReelStripGenerationConfig;
    seed: number;
    success: boolean;
    attemptsUsed: number;
