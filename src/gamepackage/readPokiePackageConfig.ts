import {PokiePackageConfig} from "pokie";
import fs from "fs";
...
export function readPokiePackageConfig(packageRoot: string): PokiePackageConfig {
    ...
    try {
        ...
        throw new Error(`Could not parse "${packageJsonPath}" as JSON: ${error instanceof Error ? error.message : String(error)}`);
    }

    const entry = packageJson.pokie?.entry;
    if (typeof entry !== "string" || entry.trim().length === 0) {
        throw new Error(`"${packageJsonPath}" is missing a "pokie.entry" field. Add e.g. {"pokie": {"entry": "./dist/index.js"}}.`);
    }
