...
export type OutcomeLibraryBundleManifestModeEntry = {
    ...
    // Present only when this mode's outcomes came from generateExactWeightedOutcomeLibrary/
    // streamExactWeightedOutcomes (see weightedoutcome/generate) rather than from an already-computed source
    // handed to OutcomeLibraryBundleModeInput directly — copied verbatim from that run's own return value,
    // never recomputed here. Absent for a mode built from a caller-supplied WeightedOutcomeLibrary/JSONL
    // stream that was never itself generated by this package.
