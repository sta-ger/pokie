...
The Dashboard has two tabs, **Overview** and **Validation**, switched client-side with no full page reload:
...
Both quick actions call `GamePackageInspecting`/`PokieGamePackageValidating` directly (the same services
`pokie inspect`/`pokie validate` use) — Studio never spawns either CLI command as a subprocess, and never
