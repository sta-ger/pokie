...
lifecycle described above. `Lifecycle required` is deliberately **not** a synchronous exemption:
it records a potentially noticeable operation whose domain executor must be moved behind that
contract before it is presented as non-blocking work. This inventory keeps that remaining
migration visible rather than treating an `async` function as evidence that the work is bounded.
...
| POST `/api/home/projects/open` | Lifecycle required: project opening/runtime materialization | Canonical requested project path is the source identity. Resolution can materialize a runtime and therefore needs a durable project-opening job rather than relying on the request-generation abort guard alone. |
...
| POST `/api/home/blueprints/par-import` | Lifecycle required: PAR import | A workbook is an external source whose parse/validation can scale with its sheets; canonical workbook path is the source identity. |
| POST `/api/home/blueprints/par-export` | Lifecycle required: PAR export | A workbook publication is atomic but can write an arbitrarily sized workbook; canonical Blueprint source and destination path own the conflict key. |
| POST `/api/home/blueprints/build` | Lifecycle required: Design package build/reel-strip materialization | Package build may generate reels and publish a destination tree. The canonical Blueprint source and requested destination identify the job. |
...
| POST `/api/project/play/sessions/:id/find-any-win`, POST `/api/project/play/sessions/:id/find-symbol-win`, POST `/api/project/play/sessions/:id/find-free-games` | Lifecycle required: Play scenario search | These routes can perform up to the configured scenario-search spin bound; session id plus immutable search request identify the job, and only a settled round may be retained. |
...
| POST `/api/project/deployment/runs` | Lifecycle required: deployment check/publish | Target, delivery request, and project root form the conflict identity; the executor must report pipeline stages and published artifact references. |
...
| POST `/api/project/certification/validate-source` | Lifecycle required: certification deep validation | A source bundle can contain an unbounded set of evidence files/modes; source identity and validation request must be retained with progress. |
| POST `/api/project/certification/build` | Lifecycle required: certification evidence build | Evidence destination, modes, and project root form the conflict identity; file/sample/mode progress and atomic publication belong to the job. |
...
The lifecycle-required rows above are the audit's migration boundary. They are intentionally
not excused merely because the current implementation awaits I/O. A later lifecycle packet must
replace each with the common job contract while retaining its existing endpoint as a compatibility
projection and retaining its domain service as executor.
