...
The candidate was built once, then Studio was started twice, each time exactly
as `node ./dist/cli/pokie.js --no-open`, with a fresh visible Chromium profile
on the inherited display. The bounded rendered-controls-only transcript is in
`current-candidate/ACTION-TRANSCRIPT.txt`.
...
dead end was observed. The native picker branch is deliberately not claimed as
passed: after one accepted visible **Select PNG** click, no picker rendered
within the bounded readiness window and no product error appeared.
