...
const output = resolve("docs/evidence/p6-05-game-model-artwork-real-workflow");
...
async function main() {
    ...
    await sectionClick("Game basics", "Edit"); await waitUntil("document.body.innerText.includes('Game id')", "Game basics editor"); await input("Game name", "P6 Saved Artwork Slot"); await sectionClick("Game basics", "Save"); await waitUntil("document.body.innerText.includes('Name: P6 Saved Artwork Slot')", "saved Game basics view"); note("OBSERVE Game basics mutation becomes dirty, validates, saves, and returns to the persisted View mode."); await snapshot("02-basics-saved");
    ...
    await sectionClick("Symbols", "Edit"); await input("Symbol 6 id", "WILD_FINAL"); await sectionClick("Symbols", "Save"); await waitUntil("document.body.innerText.includes('WILD_FINAL')", "saved Symbols view"); note("OBSERVE Symbols rename saves through validation and keeps the artwork presentation associated with the renamed symbol."); await snapshot("03-symbols-saved-artwork-visible");
    ...
    await navigate("/#/project/gameModel"); await waitUntil("document.body.innerText.includes('P6 Saved Artwork Slot') && document.querySelectorAll('img[alt=\"WILD_FINAL\"]').length >= 1", "persisted saved model and artwork after browser reload"); note("OBSERVE saved Game Model and declared PNG still render after a full browser reload."); await snapshot("07-before-server-restart");
