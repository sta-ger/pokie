...
async function main() {
    ...
    const input = async (label, value) => { await mouse(await inputPoint(label)); await cdp.send("Input.dispatchKeyEvent", {type: "keyDown", key: "a", code: "KeyA", windowsVirtualKeyCode: 65, modifiers: 2}); await cdp.send("Input.dispatchKeyEvent", {type: "keyUp", key: "a", code: "KeyA", windowsVirtualKeyCode: 65, modifiers: 2}); await cdp.send("Input.insertText", {text: value}); await sleep(250); await key("Tab", "Tab", 9); await sleep(500); note(`INPUT ${JSON.stringify(label)}=${JSON.stringify(value)} through browser keyboard`); };
    const select = async (label, value) => { await mouse(await inputPoint(label)); await cdp.send("Input.insertText", {text: value}); await key("ArrowDown", "ArrowDown", 40); await key("Enter", "Enter", 13); await key("Tab", "Tab", 9); await sleep(400); note(`SELECT ${JSON.stringify(label)}=${JSON.stringify(value)} through rendered combobox keyboard`); };
    ...
    await sectionClick("Bets & Modes", "Edit"); await input("Bet 4", "11"); await sectionClick("Bets & Modes", "Save");
    await expectSaved("Bets & Modes Save returned View with Available bets: 1, 2, 5, 11", "document.body.innerText.includes('Available bets: 1, 2, 5, 11')", "05-bets-saved");
    ...
    await expectSaved("full browser reload retained all five saved section values", "document.body.innerText.includes('Rows: 4') && document.body.innerText.includes('Generation mode: Default (uniform across symbols)') && document.body.innerText.includes('Available bets: 1, 2, 5, 11') && document.body.innerText.includes('3x → 8 free games')", "07-after-browser-reload");
