...
const output = resolve("docs/evidence/p6-02-registry-lifecycle/rerun-20260814");
const relativeExternal = "docs/evidence/p6-02-registry-lifecycle/rerun-20260814/fixtures/external-project";
const absoluteExternal = resolve(relativeExternal);
const symlinkExternal = resolve("docs/evidence/p6-02-registry-lifecycle/rerun-20260814/fixtures/external-project-alias");
const relocatedManaged = resolve("docs/evidence/p6-02-registry-lifecycle/rerun-20260814/fixtures/relocated-managed-blueprint.json");
const notes = [];
...
async function main() {
    ...
    const importProject = async (location, expectedRows, name) => {
        ...
        await wait(`document.querySelectorAll('tbody tr').length === ${expectedRows}`, `${expectedRows} rendered rows after registration`);
        note(`OBSERVE ${await evaluate("document.querySelectorAll('tbody tr').length")} rendered registry rows`);
    };
    ...
    if (phase === "1") {
        ...
        await goProjects();
        await wait("document.body.innerText.includes('Managed') && document.querySelectorAll('tbody tr').length === 1", "managed save immediately refreshed Projects without reload");
        ...
        await importProject(symlinkExternal, 2, "Registry Lifecycle External Renamed");
        await wait("document.body.innerText.includes('Registry Lifecycle External Renamed') && document.querySelectorAll('tbody tr').length === 2", "relative, absolute, and symlink canonical imports deduplicated and renamed");
        await snapshot("06-canonical-imports-and-rename");
        await click(await rowAction("Registry Lifecycle External Renamed", "Open"), "renamed external project Open", () => rowAction("Registry Lifecycle External Renamed", "Open"));
        await wait("document.body.innerText.includes('Registry Lifecycle Fixture Rerun') && document.body.innerText.includes('registry-lifecycle-fixture-rerun')", "opened external project renders fixture name and id");
        ...
        await goHome();
        await goProjects();
        await wait("document.querySelectorAll('tbody tr').length === 3", "three rows after managed, external, and artifact registration");
        await importProject(artifactPath, 3);
        await wait("document.querySelectorAll('tbody tr').length === 3", "artifact re-registration deduplicated");
        ...
        await click(await rowAction("Registry Lifecycle External Renamed", "Open"), "Open external project to refresh recency", () => rowAction("Registry Lifecycle External Renamed", "Open"));
        await wait("document.body.innerText.includes('registry-lifecycle-fixture-rerun')", "external project reopened for recency");
        await goHome();
        await goProjects();
        ...
        await wait("document.querySelector('tbody tr')?.innerText.includes('Registry Lifecycle External Renamed')", "reopened external project remains most recent after restart");
