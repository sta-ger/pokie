...
| Build/Export, Outcome Libraries Generate/Build-Export, Stake Export, PAR import/export | conversion entry points; PC-16 owns consolidation | Build / Generate / Export Stake / Exchange PAR |
...
| PC-05-CLI-03 | duplicated command rendering | PC-02: create’s suggested Build command does not quote a spaced path | Unresolved generated-command rendering | PC-06 diagnostic/output sweep | every generated command is shell-safe for the actual supplied path; regression includes spaces |
...
| PC-05-STUDIO-01 | import replacement/recovery | PC-03 frozen P2: saved-design selection can replace editable starter with invalid/blank state without explanatory import error | Unresolved Studio replacement/recovery path | PC-10 Studio recovery sweep | selection validates before replacement or retains prior design; visible recovery explains outcome |
...
| PC-05-DUP-01 | artifact conversion duplication | `build`, `generate`, `export`, specialised `par`, and Stake actions overlap | Unresolved duplicate conversion ownership | PC-16 command/conversion consolidation sweep | each user goal has one canonical route; aliases delegate with identical defaults, validation and diagnostics |
...
| PC-05-DOC-01A | documentation drift risk | `ProjectPresentation` and public docs can state prerequisites independently | Unresolved public-documentation alignment | PC-09 docs/surface sweep | docs derive/verify against shared capability contract |
