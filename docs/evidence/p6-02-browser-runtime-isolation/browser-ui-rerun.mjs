...
async function main() {
    ...
    note("NAVIGATE public Home Projects URL");
    await wait("document.body.innerText.includes('Import Project')", "rendered Projects UI");
    await scrollToPageEnd("the visible Import Project controls");
    ...
    await wait("[...document.querySelectorAll('button')].filter((e) => e.textContent?.trim() === 'Open').length >= 2", "Project B visible Open action");
    // The registry is newest-first, so this first visible Open is Project B's action.
    ...
    // Move through the browser's own visible history with physical Alt+Left input. Four history
    // steps return to the original /project/play entry created for A (B Play -> B dashboard ->
    // Home Projects -> Home Design -> A Play).
    await browserBack("B Play to B dashboard");
    await browserBack("B dashboard to Home Projects");
    await browserBack("Home Projects to Home Design");
    await browserBack("Home Design to historical A Play URL");
