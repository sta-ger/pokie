...
async function main() {
    ...
    const browserBack = async (description) => {
        await cdp.send("Input.dispatchMouseEvent", {type: "mousePressed", x: 12, y: 12, button: "back", buttons: 8, clickCount: 1});
        await cdp.send("Input.dispatchMouseEvent", {type: "mouseReleased", x: 12, y: 12, button: "back", buttons: 0, clickCount: 1});
        note(`MOUSE browser Back button: ${description}`);
        await sleep(550);
        note(`OBSERVE location after Back: ${await evaluate("location.href")}`);
    };
    const browserForward = async (description) => {
        await cdp.send("Input.dispatchMouseEvent", {type: "mousePressed", x: 12, y: 12, button: "forward", buttons: 16, clickCount: 1});
        await cdp.send("Input.dispatchMouseEvent", {type: "mouseReleased", x: 12, y: 12, button: "forward", buttons: 0, clickCount: 1});
        note(`MOUSE browser Forward button: ${description}`);
        await sleep(550);
    ...
    // These four physical steps are the actual history sequence observed in this fresh Studio:
    // B Play -> B Overview -> Home Projects -> Home Design -> A Play.
    for (let step = 1; step <= 4; step++) await browserBack(`history step ${step} from Project B`);
    const restoredA = await evaluate("document.body.innerText.includes('Browser Multi-mode Library')");
    note(`OBSERVE Project A visible after browser Back: ${restoredA}`);
    await snapshot("08-browser-back-historical-a-route");

    for (let step = 1; step <= 4; step++) await browserForward(`history step ${step} toward Project B`);
    const restoredB = await evaluate("document.body.innerText.includes('Playable Game With Bonus Round')");
    note(`OBSERVE Project B visible after browser Forward: ${restoredB}`);
    await snapshot("09-browser-forward-project-b-route");
    if (!restoredA) throw new Error("Historical Project A route rendered a different project after browser Back.");
