...
- **`CustomLinesDefinitions`** — hand-authored paylines:
  ...
  ```
- **`LinesDefinitionsFor3x3` / `LinesDefinitionsFor5x3` / `LinesDefinitionsFor5x4`** — static preset tables of
  commonly-seen payline layouts: 11, 25, and 16 lines respectively, built from recognizable shape families (flat, V,
  M/W arch, staircase, zigzag, notch). Use these when your grid size matches exactly; otherwise use
  `HorizontalLines` or `CustomLinesDefinitions`.
- **`WaysDefinitions(reelsNumber, reelsSymbolsNumber)`** — models "ways to win" as fixed lines: every possible row
  combination across reels counts as a line (a symbol anywhere in each reel matches). Line ids here are just array
  indices, not meaningful identifiers, and the definition count is `reelsSymbolsNumber ** reelsNumber` (e.g. 4 rows ×
  5 reels = 1024 — the classic "1024 ways" games). Pair with `LeftToRightLinesPatterns` for the usual ways
  convention (match from reel 1). Watch the combinatorial growth for larger grids — for large "ways" games, prefer
  `WaysWinCalculator` (see [Paytable & Win Calculation](paytable-and-wins.md#ways-to-win)), which computes the same
  result without enumerating every line as a discrete combination.
