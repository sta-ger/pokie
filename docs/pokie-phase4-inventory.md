...
`P4-POLISH-02`'s own surface, `serve`/`dev`/`client` (the player/preview commands) are `P4-POLISH-09`'s, the
Studio server-side project-open route is `P4-POLISH-11`'s, and the `pokie-integration` real-`npm install` gap is
`P4-POLISH-04`'s — see "Owner steps" below for the full mapping and its rationale.
...
case** (verified: no match for `with space`/`isTTY`/`isatty` in the file) — owned by `P4-POLISH-11` (Studio Play),
...
**Owner-step mapping used below:** this step's own instruction names the roadmap's remaining journeys — create,
materialization, Replay, player, Studio Play, help/`pokie-examples`, …, final browser acceptance — each of which
...
- **CLI paths-with-spaces/non-TTY coverage** beyond `dispatch()`/materialize/replay (§1) — `sim`, `validate`, and
  `inspect` are `P4-POLISH-02`'s own surface; `serve`, `dev`, and `client` are `P4-POLISH-09`'s; Studio's
  server-side project-open routes are `P4-POLISH-11`'s. None of the three has a dedicated space-path regression
...
- **Integration-lane (real `npm install`) space-path coverage** for materialization
  ...
  `GamePackagePreparer.integration.test.ts`'s own existing case — owned by `P4-POLISH-04` (materialization).
- **Final player/browser acceptance matrix** — §4 now contains a real host-side baseline capture for the
  ...
  Studio-server HTTP coverage remains in `tests/cli/studio/StudioServer.test.ts` (§4), `P4-POLISH-11`'s (Studio
  Play) own surface.
...
- **CLI/Studio-server paths-with-spaces/non-TTY coverage on the Studio HTTP surface itself** — confirmed absent
  ...
  `P4-POLISH-11` (Studio Play), the same step that owns Studio's server-side project-open routes above.
