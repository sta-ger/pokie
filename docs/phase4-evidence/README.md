...
| --- | --- |
| `pokie-examples-checkout.txt` | A real `git clone --branch develop` of `github.com/sta-ger/pokie-examples`, pinned commit `af432206a435db5c1063ca5cd9dd81652b886a6e` (2026-07-09), full `src/**/*.ts` file tree with line counts, `package.json`, and five source spot-checks re-confirming §6's own claims — the actual source §6's classification table was read from, not a representative-file sample over `raw.githubusercontent.com`. Replaces a prior version of this artifact that had cloned the default (`main`) branch at a different commit while the inventory's own prose already claimed `develop` — caught and fixed this correction round; both branches' `src/` trees are byte-identical, so no classification changed. |
