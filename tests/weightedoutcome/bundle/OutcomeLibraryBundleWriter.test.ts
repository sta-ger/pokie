...
describe("OutcomeLibraryBundleWriter", () => {
    ...
    it("rejects empty, external, and symlinked directories before direct bundle staging", async () => {
        const writer = new OutcomeLibraryBundleWriter("1.3.0");
        fs.mkdirSync(outDir);
        await expect(writer.writeToDirectory(modes(), outDir)).rejects.toThrow(/was not generated by "pokie outcomelibrary build"/);
        ...
        const externalDir = `${outDir}-external`;
        fs.mkdirSync(externalDir);
        fs.rmSync(outDir, {recursive: true, force: true});
        fs.symlinkSync(externalDir, outDir, "dir");
        await expect(writer.writeToDirectory(modes(), outDir)).rejects.toThrow(/symbolic link/i);
    ...
    it("restores the old outDir byte-for-byte when the publish rename fails after the old directory was moved aside", async () => {
        ...
        // The final two calls are the private staging probe, rather than a
        // reversible sequence of live-directory renames.
        expect(renameCallCount).toBe(5);
        expect(fs.readdirSync(outDir).sort()).toEqual(filesBefore);
        expect(siblingLeftovers(outDir)).toEqual([]);
    });
