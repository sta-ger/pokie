...
describe("SimCommand runtime package materialization boundary", () => {
    ...
    it.each([
        ...
        const workDir = fs.mkdtempSync(path.join(os.tmpdir(), "pokie-sim-wasm-boundary-"));
        ...
        const command = new SimCommand(loadGame, writeFile, undefined, undefined, undefined, resolveRuntimePackageRoot);

        try {
            await expect(command.run([wasmPath, "--rounds", "1", "--out", reportPath])).rejects.toThrow(expectedDiagnostic);
