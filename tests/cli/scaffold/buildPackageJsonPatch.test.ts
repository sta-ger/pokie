...
describe("buildPackageJsonPatch", () => {
    it("sets pokie.entry, and fills in build/sim scripts and dependencies on a bare package.json", () => {
        ...
        expect(patched.scripts).toEqual({build: "tsc", sim: "pokie sim"});
    ...
    it("does not clobber existing scripts, dependency versions, or pokie config sub-fields", () => {
        ...
        expect(patched.scripts).toEqual({build: "webpack", test: "jest", sim: "pokie sim"});
