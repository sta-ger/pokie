...
describe("StudioSimulationService (integration, real worker threads via --workers)", () => {
    ...
    // waitForTerminal (above) polls via a bare setImmediate — fine for the fake-driven tests above,
    // where progress advances in lockstep with queued microtasks, but real worker threads take real
    // wall-clock time to spin up/load/compute, which thousands of back-to-back setImmediate ticks
    // don't reliably span. This polls with a real delay instead.
    ...
    it("cancelling a workers>1 job stops it (and its worker threads) without returning a partial report as successful", async () => {
        ...
        const result = service.start(fixtureRoot, {rounds: 5_000_000, workers: 2});
        if (result.status !== "created") {
            throw new Error("expected job to be created");
        }

        // Give the workers a moment to actually start before cancelling.
        await new Promise((resolve) => {
