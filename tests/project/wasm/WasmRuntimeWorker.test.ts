...
describe("PokieWasmWorkerProtocol", () => {
    it("owns transfer-safe bytes, host-issued draws, state restoration, and disposal", async () => {
        ...
        expect(await protocol.handle({id: "start", type: "instantiate", bytes: fixture.bytes, manifest: fixture.manifest, draws: [0.25, 0.75]})).toMatchObject({ok: true});
    ...
    it("cancels acquired resources and rejects malformed protocol state", async () => {
        ...
        const protocol = new PokieWasmWorkerProtocol();
        await expect(protocol.handle({id: "start", type: "instantiate", bytes: fixture.bytes, manifest: fixture.manifest, draws: [0.25]})).resolves.toMatchObject({ok: true});
