...
async function driveRoundTrip(gatePath, gateSha256, {readFile:read = readFile, fetch:request = fetch, environment = process.env, now = () => new Date().toISOString()} = {}) {
    ...
    return {uploaded:true, readBack:true, releaseGateSha256:gateSha256, readBackSha256:gateSha256, uploadId:uploaded.id, uploadedAt:now(), readBackAt:now()};
}
