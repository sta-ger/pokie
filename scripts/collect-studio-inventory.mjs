...
async function main() {
    ...
    const processEnvironment = {...process.env, XDG_CONFIG_HOME: config, XDG_CACHE_HOME: join(temporaryRoot, "cache")};
    let studio;
    let browser;
    let cdp;
    try {
        ...
        note(`Fresh profile=${profile}; fresh Studio config=${config}; public entrypoint=http://127.0.0.1:${PORT}/`);
        ...
        const snapshot = async (goal) => {
            ...
            return {goal, latencyMs: Date.now() - started, ...view, focusOrder: focus};
        };
        ...
        const screens = [await snapshot("First-time creator arrives at Studio")];
        await click("New Blueprint");
        screens.push(await snapshot("Creator chooses a new Blueprint"));
        ...
        screens.push(await snapshot("Creator configures the recommended Blueprint"));
        await click("Create Project");
        await waitFor(() => evaluate("document.body.innerText.includes('Close project')"), "created Blueprint project dashboard", 60_000);
        screens.push(await snapshot("Created project overview"));
        ...
        for (const tab of tabs) {
            await click(tab);
            screens.push(await snapshot(`Project tab: ${tab}`));
        }
        await click("Projects");
        screens.push(await snapshot("Projects registry after creating a project"));
        const inventory = {
            schemaVersion: 1,
